-- Database
CREATE DATABASE "MSI_CollectionsDB"
       WITH
       OWNER = postgres
       ENCODING = 'UTF8'
       LC_COLLATE = 'C'
...
CREATE TABLE dialer_interactions (
    interaction_id BIGSERIAL PRIMARY KEY AUTO_INCREMENT,
    date    DATE NOT NULL,
...
CREATE TABLE ptp_log (
    ptp_id  BIGSERIAL PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE cures_log (
    cure_id BIGSERIAL PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE payment_schedule ( -- quizás linkear con ptp, no se si necesario
    schedule_id BIGSERIAL PRIMARY KEY AUTO_INCREMENT,
    account_id INT NOT NULL,
...
CREATE TABLE agent_time_log (
    time_id BIGSERIAL PRIMARY KEY AUTO_INCREMENT,
    date DATE NOT NULL,
    agent_id    INT NOT NULL,
