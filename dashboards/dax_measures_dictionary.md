...
| **Total Amount Paid** | `SUM('Fact_Payments'[amount_paid])` |
...
| **Cured Amount Prior Month** | `CALCULATE([Total Amount Paid], DATEADD('Dim_Calendar'[date], -1, MONTH))` |
| **Cured Amount MoM %** | `DIVIDE([Total Amount Paid] - [Cured Amount Prior Month], [Cured Amount Prior Month], 0)` |
...
| **Cured Amount YTD** | `CALCULATE([Total Amount Paid], DATESYTD('Dim_Calendar'[date]))` |
