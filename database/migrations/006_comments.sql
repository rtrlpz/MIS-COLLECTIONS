...
COMMENT ON COLUMN dim_clients.income_bracket IS 'Income bracket: <30K, 30K-50K, 50K-75K, 75K-100K, 100K+ (G4)';
...
COMMENT ON COLUMN dim_accounts.initial_balance IS 'Starting balance at collections window entry';
...
COMMENT ON COLUMN fact_interactions.channel IS 'Interaction channel: Dialer, Manual, FICO, SMS (G5)';
...
COMMENT ON COLUMN fact_agent_time_log.utilization IS 'Utilization ratio: THT / Operational Hours (percentage 0-100)';
COMMENT ON COLUMN fact_agent_time_log.schedule_hours IS 'Scheduled shift hours for the day';
COMMENT ON COLUMN fact_agent_time_log.cost_per_hour IS 'Agent hourly cost rate (G9 — by experience tier)';
COMMENT ON COLUMN fact_agent_time_log.total_cost IS 'Total daily cost: cost_per_hour * operational_hours * overhead (G9)';
...
-- ============================================================================
-- NEW TABLE: Fact_Writeoffs (G6)
-- ============================================================================

COMMENT ON TABLE fact_writeoffs IS 'Write-off events when accounts reach 91+ DPD (G6 — Financial Recovery page)';
COMMENT ON COLUMN fact_writeoffs.writeoff_id IS 'Unique write-off identifier (format: WOF_XXXXX)';
COMMENT ON COLUMN fact_writeoffs.writeoff_date IS 'Date write-off was processed (FK to dim_calendar)';
COMMENT ON COLUMN fact_writeoffs.account_id IS 'FK to dim_accounts — account being written off';
COMMENT ON COLUMN fact_writeoffs.product_type IS 'Product type at time of write-off (denormalized)';
COMMENT ON COLUMN fact_writeoffs.writeoff_amount IS 'Amount written off (full arrears at 91+ DPD)';
COMMENT ON COLUMN fact_writeoffs.balance_before IS 'Outstanding balance before write-off';
COMMENT ON COLUMN fact_writeoffs.dpd_at_writeoff IS 'Days Past Due when write-off occurred';
